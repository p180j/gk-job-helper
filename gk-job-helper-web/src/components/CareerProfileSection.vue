...
function addFact(): void { factEditor.value = { factType: 'SKILL_EXPERIENCE', factKey: '', valueType: 'NUMBER', numberValue: null, unit: 'YEAR', displayName: '' } }
...
<template>
  <section class="page-card career-card">
    ...
    <div class="fact-head"><h3>已确认职业信息</h3><el-button size="small" type="primary" plain @click="addFact">补充职业信息</el-button></div>
    <p class="fact-tip">只保存可重复使用的客观事实，例如 Java 开发年限、行业年限、证书或职称；保存后会自动重新判断招聘岗位。</p>
    ...
    <el-empty v-if="!careerFacts.length && !factEditor" description="暂无已确认职业信息" :image-size="56" />
