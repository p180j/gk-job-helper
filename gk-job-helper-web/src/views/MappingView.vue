...
const result = ref<ImportResult | null>(null); const progress = ref<ImportProgress | null>(null); const loading = ref(false)
...
async function submit() { if (!mappings.value.some(item => item.targetField === 'positionName')) { ElMessage.warning('岗位名称是关键字段，请至少映射“岗位名称”。'); return }; loading.value = true; try { await confirmImport(importId, mappings.value); await pollProgress() } catch (error) { showError(error, '职位表导入失败。') } finally { loading.value = false } }
...
<template>
  <section class="page-card"><h1 class="page-title">确认字段映射</h1><p class="page-subtitle">Sheet：{{ data?.sheetName ?? '-' }}。可修改系统建议；“岗位名称”为关键字段。</p>
    ...
    <div v-if="!result" class="actions"><el-button type="primary" :loading="loading" @click="submit">确认导入职位</el-button></div>
...
<style scoped>.key-tag{margin-left:8px}.actions{margin-top:22px;text-align:right}.progress,.result{margin-top:18px}</style>
