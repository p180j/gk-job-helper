...
import { executeMatch } from '@/api/match'
...
onMounted(async () => { try { const profile = await fetchProfile(); if (!profile) return; summary.value = await executeMatch(profile.id, importId, new Date().toISOString().slice(0, 10)) } catch (error) { if (isProfileNotFound(error)) router.replace('/profile'); else showError(error, '岗位匹配失败，请稍后重试。') } finally { loading.value = false } })
