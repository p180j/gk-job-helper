...
public class HistoricalAnalysisService {
 ...
 public HistoricalAnalysisVO analyze(JobPosition current,JobPositionFeature feature,JobInterviewScore score,List<HistoricalSampleRow> source,Integer requestedYear){if(feature==null||feature.getExamSubjectGroup()==null)return empty(requestedYear,"岗位考试科目尚未可靠识别");
  ...
  List<HistoricalSampleRow> same=new ArrayList<>();for(HistoricalSampleRow r:unique.values())if(feature.getExamSubjectGroup().equals(r.getExamSubjectGroup())&&!samePosition(current,r))same.add(r);
  ...
  vo.setRankingEligible(("LEVEL1".equals(level)||"LEVEL2".equals(level)||"LEVEL3".equals(level))&&!"LOW".equals(confidence)&&vo.getPercentile()!=null);vo.setComparisonDescription("同类岗位样本 "+chosen.size()+" 个，平均分 "+average.toPlainString()+"，中位数 "+median.toPlainString());return vo;
 }
 private boolean samePosition(JobPosition current,HistoricalSampleRow row){return current!=null&&current.getId()!=null&&current.getId().equals(row.getPositionId());}
 private List<HistoricalSampleRow> filter(List<HistoricalSampleRow> rows,JobPosition p,JobPositionFeature f,int level){List<HistoricalSampleRow> out=new ArrayList<>();for(HistoricalSampleRow r:rows){boolean similar=similarityOverlap(f.getMajorRestrictionType(),f.getMajorSimilarityKeys(),r.getMajorRestrictionType(),r.getMajorSimilarityKeys());if(level<=2&&!similar)continue;if(level==3)continue;if(level==1&&(!family(f.getMajorRestrictionType()).equals(family(r.getMajorRestrictionType()))||bucket(p.getRecruitCount())!=bucket(r.getRecruitCount())))continue;out.add(r);}return out;}
 private boolean similarityOverlap(String ta,String ka,String tb,String kb){if("UNRESTRICTED".equals(ta)&&"UNRESTRICTED".equals(tb))return true;Set<String>a=new HashSet<>(list(ka)),b=new HashSet<>(list(kb));a.retainAll(b);return !a.isEmpty();}
 ...
 private HistoricalAnalysisVO empty(Integer year,String desc){HistoricalAnalysisVO v=new HistoricalAnalysisVO();v.setExamYear(year);v.setAvailable(false);v.setConfidence("LOW");v.setReliable(false);v.setRankingEligible(false);v.setComparisonDescription(desc);return v;}
}
