...
public class RecruitmentAutoProcessService {
    ...
    private final RecruitmentPositionService positions;
    ...
    public RecruitmentAutoProcessService(RecruitmentDetailService details, RecruitmentPositionService positions,
            RecruitmentQualificationMatchService matches, RecruitmentNoticeService notices,
            RecruitmentNoticeMapper noticeMapper, RecruitmentAttachmentMapper attachments, RecruitmentQrResolver qrResolver) {
    ...
    public void processNotice(Long noticeId) {
        ...
        try {
            ...
            List<RecruitmentAttachment> qrHints = attachments.selectQrAttachmentHintsByNoticeId(noticeId);
            boolean hasSavedSource = !files.isEmpty() || !qrHints.isEmpty()
                    || (notice.getBodyHtml() != null && !notice.getBodyHtml().trim().isEmpty())
                    || (notice.getBodyText() != null && !notice.getBodyText().trim().isEmpty());
            if (!hasSavedSource && !"FETCHED".equals(notice.getDetailStatus())) details.fetch(noticeId);
            notice = notices.detail(noticeId);
            files = attachments.selectPositionDataByNoticeId(noticeId);
            qrHints = attachments.selectQrAttachmentHintsByNoticeId(noticeId);
            noticeMapper.updateProcess(noticeId, "PROCESSING", "SOURCE_DETECT", null, null, 0, null, LocalDateTime.now());
            int count = 0;
            if (!files.isEmpty()) {
                count = positions.extract(noticeId).getPositionCount();
            } else if (!qrHints.isEmpty()) {
                noticeMapper.updateProcess(noticeId, "PROCESSING", "QR_RESOLVE", null, null, 0, null, LocalDateTime.now());
                RecruitmentQrResolver.Resolution resolution = qrResolver.resolve(noticeId);
                files = attachments.selectPositionDataByNoticeId(noticeId);
                if (!files.isEmpty()) {
                    count = positions.extract(noticeId).getPositionCount();
                } else if (resolution.qrCount > 0) {
                    manual(noticeId, resolution.caoliaoUnresolvedCount > 0 ? "CAOLIAO_ATTACHMENT_LIST_UNRESOLVED" : "QR_MANUAL",
                            resolution.caoliaoUnresolvedCount > 0 ? "草料文件页未提取到可下载附件" : "二维码未发现可自动解析的岗位附件");
                    return;
                }
            } else if (notice.getBodyPositionHint() != null) {
                count = positions.extractBody(noticeId).getPositionCount();
            } else {
                manual(noticeId, "NO_VALID_POSITION", "未发现可自动结构化的岗位来源");
                return;
            }
            if (count > 0) {
                matchPositions(noticeId);
                noticeMapper.updateProcess(noticeId, "COMPLETED", "DONE", null, null, count, LocalDateTime.now(), LocalDateTime.now());
            } else {
                manual(noticeId, "NO_VALID_POSITION", "未识别到有效岗位");
            }
        } catch (Exception exception) {
            String code = manualCode(noticeId, exception.getMessage());
            if ("PARSE_FAILED".equals(code)) {
                noticeMapper.updateProcess(noticeId, "FAILED", "POSITION_EXTRACT", code, safe(exception.getMessage()), 0, null, LocalDateTime.now());
            } else {
                manual(noticeId, code, safe(exception.getMessage()));
            }
        }
    }
    ...
    private String manualCode(Long id, String message) {
        ...
        if (text.contains("安全验证") || text.contains("EXTERNAL_SOURCE_ACCESS_RESTRICTED")) return "EXTERNAL_SOURCE_ACCESS_RESTRICTED";
