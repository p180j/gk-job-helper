...
CREATE TABLE IF NOT EXISTS career_fact (
    ...
    boolean_value TINYINT DEFAULT NULL,
    number_value DECIMAL(12,2) DEFAULT NULL,
    text_value VARCHAR(1000) DEFAULT NULL,
    unit VARCHAR(32) DEFAULT NULL,
    display_name VARCHAR(255) NOT NULL,
    source VARCHAR(32) NOT NULL DEFAULT 'USER_CONFIRMED',
    confirmed_at DATETIME DEFAULT NULL,
    created_at DATETIME DEFAULT NULL,
    updated_at DATETIME DEFAULT NULL,
    PRIMARY KEY(id),
    UNIQUE KEY uk_career_fact_profile_key(profile_id,fact_type,fact_key),
    KEY idx_career_fact_profile(profile_id)
);
...
CREATE TABLE IF NOT EXISTS import_file (
    ...
    exam_year        INT           DEFAULT NULL,            -- 岗位表考试年度（从原始文件名识别）
...
-- Iteration 7：岗位收藏；不复制岗位数据，直接关联档案与岗位。
...
CREATE UNIQUE INDEX uk_job_favorite_profile_position ON job_favorite (profile_id, position_id);
CREATE INDEX idx_job_favorite_profile_created ON job_favorite (profile_id, created_at);
...
INSERT IGNORE INTO recruitment_source(source_code,source_name,source_type,list_url,enabled,created_at,updated_at) VALUES ('JIANGXI_TALENT_EVALUATION','江西省人才测评中心','WEB','https://www.jxrcrsksta.com/list/7',1,NOW(),NOW());
CREATE TABLE IF NOT EXISTS recruitment_notice (id BIGINT NOT NULL AUTO_INCREMENT,source_id BIGINT NOT NULL,title VARCHAR(1000) NOT NULL,notice_url VARCHAR(512) NOT NULL,publish_date DATETIME DEFAULT NULL,notice_type VARCHAR(32) NOT NULL,parent_notice_id BIGINT DEFAULT NULL,user_status VARCHAR(32) NOT NULL,detail_status VARCHAR(16) NOT NULL DEFAULT 'DISCOVERED',process_status VARCHAR(16) NOT NULL DEFAULT 'DISCOVERED',process_stage VARCHAR(32) DEFAULT NULL,process_failure_code VARCHAR(64) DEFAULT NULL,process_failure_reason VARCHAR(500) DEFAULT NULL,process_position_count INT NOT NULL DEFAULT 0,processed_at DATETIME DEFAULT NULL,body_html LONGTEXT DEFAULT NULL,body_text LONGTEXT DEFAULT NULL,detail_fetched_at DATETIME DEFAULT NULL,detail_error VARCHAR(500) DEFAULT NULL,viewed_at DATETIME DEFAULT NULL,discovered_at DATETIME NOT NULL,created_at DATETIME DEFAULT NULL,updated_at DATETIME DEFAULT NULL,PRIMARY KEY(id),UNIQUE KEY uk_recruitment_notice_source_url(source_id,notice_url),KEY idx_recruitment_notice_status(user_status),KEY idx_recruitment_notice_process(process_status),CONSTRAINT fk_recruitment_notice_source FOREIGN KEY(source_id) REFERENCES recruitment_source(id));
...
CREATE TABLE IF NOT EXISTS recruitment_match_result (id BIGINT NOT NULL AUTO_INCREMENT,profile_id BIGINT NOT NULL,position_id BIGINT NOT NULL,match_result VARCHAR(16) NOT NULL,matched_count INT NOT NULL DEFAULT 0,uncertain_count INT NOT NULL DEFAULT 0,not_matched_count INT NOT NULL DEFAULT 0,summary VARCHAR(1000) DEFAULT NULL,uncertain_reason VARCHAR(64) DEFAULT NULL,created_at DATETIME DEFAULT NULL,updated_at DATETIME DEFAULT NULL,PRIMARY KEY(id),UNIQUE KEY uk_recruitment_match_profile_position(profile_id,position_id),KEY idx_recruitment_match_profile_result(profile_id,match_result));
CREATE TABLE IF NOT EXISTS recruitment_match_item (id BIGINT NOT NULL AUTO_INCREMENT,match_result_id BIGINT NOT NULL,requirement_id BIGINT DEFAULT NULL,requirement_type VARCHAR(32) NOT NULL,requirement_level VARCHAR(16) NOT NULL,result VARCHAR(16) NOT NULL,position_requirement TEXT,user_evidence TEXT,reason VARCHAR(1000) DEFAULT NULL,created_at DATETIME DEFAULT NULL,PRIMARY KEY(id),KEY idx_recruitment_match_item_result(match_result_id));
